...
func (s *PersisterTestSuite) TestFindSubjectsGrantedConsentRequests() {
	...
	for k, r := range s.registries {
		t.Run(k, func(t *testing.T) {
			...
			actual, err := r.Persister().FindSubjectsGrantedConsentRequests(s.t2, f.Subject, consent.AllActive, 100, 0)
			require.Error(t, err)
			require.Equal(t, 0, len(actual))

			actual, err = r.Persister().FindSubjectsGrantedConsentRequests(s.t1, f.Subject, consent.AllActive, 100, 0)
			require.NoError(t, err)
			require.Equal(t, 1, len(actual))
		})
	}
