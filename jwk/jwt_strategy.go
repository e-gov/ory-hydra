...
func (j *RS256JWTStrategy) refresh(ctx context.Context) error {
	if j.c.HsmEnabled() {
		if keyPair, err := j.r.HardwareSecurityModule().FindKeyPair([]byte(j.c.HsmKeyId()), nil); err != nil {
			return errors.WithMessagef(err,"Unable to configure signing key. Library path: %s, Slot: %v, Token label: %s Signing key id: %s",
				j.c.HsmLibraryPath(), j.c.HsmSlotNumber(), j.c.HsmTokenLabel(), j.c.HsmKeyId())
		} else {
			j.Lock()
			j.RS256JWTStrategy.PrivateKey = cryptosigner.Opaque(keyPair)
			j.publicKey = keyPair.Public().(crypto.PublicKey).(*rsa.PublicKey)
			j.publicKeyID = j.c.HsmKeyId()
			j.Unlock()
			j.RLock()
			defer j.RUnlock()
		}
